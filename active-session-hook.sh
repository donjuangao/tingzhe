...
⛔ **那句话必须有内容 —— 空话不算数**(合格线由外部验收卷 C8 给出,不是我自己定的):
· ✅ 合格 = **承诺未来 + 说出你此刻的判断**,但不断言只有工具才知道的事实
  例:「这个我要跑几个检查、大概一分钟;我先说我的判断:多半是 X 那条路,因为 Y」
· ❌ 不合格① = 工具还没跑完,就用确定语气说出只有工具结果才知道的事实/数字/结论
· ❌ 不合格② = **纯填充**(「好的,我看一下」「稍等」)——技术上没说错,但需求要的是
  「先把话说回去」,不是「先发一个占位音」

⚠ **实测:那 4 分钟沉默的成因就是你在串行等工具**(作者 逐字:「271 秒你沉默是不是因为你在等工具链反馈」)。
所以派完之后**别闭嘴**:工兵在飞的时候,把你正在想的、下一步打算怎么判,继续说给他听。
**沉默才是那个 271 秒,不是工具本身。**
